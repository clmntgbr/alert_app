...
class ApiService {
  ...
  Future<GetActiveStore> getActiveStore() async {
    ...
    if (response.statusCode == 200) {
      GetActiveStore model = activeStoreFromJson(response.body.toString());
      return model;
    }
  ...
  Future<GetItems> getItems() async {
    String storeIsActive = 'store.isActive=true';
    if (globals.storeActiveId > -1) {
      storeIsActive = 'store.id=${globals.storeActiveId}';
    }

    debugPrint('GET ${ApiConstants.baseUrl}${ApiConstants.itemsEndpoint}?$storeIsActive');

    Response response = await get(Uri.parse('${ApiConstants.baseUrl}${ApiConstants.itemsEndpoint}?$storeIsActive'),
        headers: {'Content-Type': 'application/ld+json', 'Accept': 'application/ld+json', 'Authorization': token});

    if (response.statusCode == 200) {
      GetItems model = itemsFromJson(response.body.toString());
      return model;
    }
